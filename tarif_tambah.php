<?php 
 ...
 $i = mysql_fetch_array(mysql_query("SELECT * FROM tbl_tarif ORDER BY id_tarif DESC LIMIT 0,1"));

    $id_awal = substr($i['id_tarif'],3,4)+1;
    if ($id_awal < 10) {
        $id = '4000'.$id_awal;
    ...
    $j = mysql_fetch_array(mysql_query("SELECT * FROM tbl_tarif ORDER BY kode DESC LIMIT 0,1"));

    $id_awal1 = substr($j['kode'],3,4)+1;
    if ($id_awal1 < 10) {
        $id1 = 'KD00'.$id_awal1;
    } else if (($id_awal > 9) && ($id_awal1 <= 99)) {
        $id1 = 'KD0'.$id_awal1;
    } else {
        $id1 = 'KD0'.$id_awal1;
    }
...
<div id="templatemo_wrapper">
  ...
  <?php if(($_SESSION['TypeUser']=='admin') || ($_SESSION['TypeUser']=='user')) { ?>
    ...
    <div id="templatemo_middle" class="carousel">
        ...
        <center>
          <div id="tambahubah">
            <p>Tambah Data Tarif</p>
               <form name="tambahubah" method="post" action="tarif_tambah_proses.php">
                   
                   <div class="group-input" style="margin-bottom: 15px;">
                                            <label>ID Tarif :</label>
                                            <input type="text" name="id_tarif" class="form-control" readonly="" value="<?php echo $id; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <label>Kode :</label>
                                            <input type="text" name="kode" class="form-control" readonly="" value="<?php echo $id1; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            ...
                                            <input type="text" name="daya" class="form-control" required="" placeholder="Daya Terpakai">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
