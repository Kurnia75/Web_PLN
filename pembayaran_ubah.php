      ...
      <?php } ?>
        ...
        <div id="menu_second_bar">
        	...
        	<div id="templatemo_search">
				<form action="hasil_pencarian_jenisbiaya.php" method="post" id="sitesearch">
				<input type="text" value="Cari Data" name="NamaBiaya" id="NamaBiaya" title="NamaBiaya" onfocus="clearText(this)" onblur="clearText(this)" class="txt_field" />
    ...
    <div id="templatemo_middle" class="carousel">
        ...
        <div id="content" class="float_r">
            ...
            <p>Ubah Data Pembayaran</p>
                <form name="tambahubah" method="post" action="pembayaran_ubah_proses.php">
                   <div class="group-input" style="margin-bottom: 15px;">
                                            <label>ID Pembayaran :</label>
                                            <input type="text" name="id_pembayaran" class="form-control" readonly="" value="<?php echo $data1['id_pembayaran']; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <label>ID Tagihan :</label>
                                            <select name="id_tagihan" class="form-control" required=""> 
                                                <option value="<?php echo $data1['id_tagihan']; ?>">Default : <?php echo $data1['id_tagihan'];?></option>
                                                <?php 
                                                    $query = mysql_query("SELECT * FROM tbl_tagihan"); 
                                                    while($data = mysql_fetch_array($query)){ 
                                                    echo "<option value='$data[id_tagihan]'> $data[id_tagihan] </option>"; 
                                                    } 
                                                ?>
                                            </select>
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            ...
                                            <input type="date" name="tanggal_bayar" class="form-control" required="" value="<?php echo $data1['tanggal_bayar']; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <select name="pemakaian" required>
                            <option value="">Jumlah Tagihan:</option>
                            <?php 
                                mysql_connect("localhost","root",""); 
                                mysql_select_db ("db_pln");
                                $query = mysql_query("SELECT * FROM tbl_tagihan"); 
                                while($data = mysql_fetch_array($query)){ 
                                echo "<option value='$data[pemakaian]'> $data[pemakaian] </option>"; 
                                } 
                            ?>
                        </select>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <label>Biaya Denda :</label>
                                            <input type="text" name="biaya_denda" class="form-control" required="" value="<?php echo $data1['biaya_denda']; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <label>Biaya Admin</label>
                                            <input type="text" name="biaya_admin" class="form-control" required="" value="<?php echo $data1['biaya_admin']; ?>">
                                        </div>
                                        <div class="group-input" style="margin-bottom: 15px;">
                                            <label>Status Pembayaran :</label>
                                            <select name="status_pembayaran" required="" class="form-control">
                                                <option value="<?php echo $data1['status_pembayaran']; ?>">Default : <?php echo $data1['status_pembayaran']; ?></option>
